...
int StridedSliceGrad::InferShape(std::vector<lite::Tensor *> inputs, std::vector<lite::Tensor *> outputs) {
  ...
  for (size_t i = 0; i < ndim_; i++) {
    begins_mask_.at(i) = static_cast<bool>(GetBeginMask()) & (1ul << i);
    ends_mask_.at(i) = static_cast<bool>(GetEndMask()) & (1ul << i);
    ellipsis_mask_.at(i) = static_cast<bool>(GetEllipsisMask()) & (1ul << i);
    new_axis_mask_.at(i) = static_cast<bool>(GetNewAxisMask()) & (1ul << i);
    shrink_axis_mask_.at(i) = static_cast<bool>(GetShrinkAxisMask()) & (1ul << i);
  }
