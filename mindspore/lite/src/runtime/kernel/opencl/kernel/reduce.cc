...
int ReduceOpenCLKernel::SetAxes() {
  ...
  if (reduce_param->num_axes_ > 2 || reduce_param->num_axes_ < 1) {
    MS_LOG(ERROR) << "Unsupported reduce axes " << reduce_param->num_axes_;
    return RET_PARAM_INVALID;
  }
  for (int i = 0; i < reduce_param->num_axes_; i++) {
    int axis = reduce_param->axes_[i];
    axis = inShape.AlignAxis(axis);
    reduce_axes_[axis] = true;
  }
  if (reduce_param->num_axes_ == 1) {
