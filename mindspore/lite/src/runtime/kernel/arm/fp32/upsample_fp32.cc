...
int UpsampleCPUKernel::ReSize() {
  ...
  if (param_->method_ == 0) {  // bilinear
    ...
    ret =
      PrepareResizeBilinear(input_shape.data(), output_shape.data(), CalculateAsymmetric, coordinate_.y_bottoms_,
                            coordinate_.y_tops_, coordinate_.x_lefts_, coordinate_.x_rights_, y_weights_, x_weights_);
    if (ret != RET_OK) {
      FreeTmpBuffer();
    }
  }
...
int UpsampleCPUKernel::RunImpl(int task_id) {
  ...
  switch (param_->method_) {
    case static_cast<int>(schema::ResizeMethod_LINEAR): {
      ...
      ret = ResizeBilinear(input_data, output_data, input_shape.data(), out_tensor->shape().data(),
                           coordinate_.y_bottoms_, coordinate_.y_tops_, coordinate_.x_lefts_, coordinate_.x_rights_,
                           y_weights_, x_weights_, line0, line1, n_h_begin, n_h_end);
      break;
    }
