...
const BaseRef OnnxGeLUFusion::DefinePattern() const {
  VectorRef div_ref({std::make_shared<CondVar>(IsSpecifiedNode<schema::PrimitiveType_Div>), input_, div_y_});
  VectorRef erf_ref({std::make_shared<CondVar>(IsSpecifiedNode<schema::PrimitiveType_Erf>), div_ref});
  VectorRef add_ref({std::make_shared<CondVar>(IsSpecifiedNode<schema::PrimitiveType_Add>), erf_ref, add_y_});
  VectorRef mul1_ref({std::make_shared<CondVar>(IsSpecifiedNode<schema::PrimitiveType_Mul>), input_, mul1_y_});
  VectorRef mul2_ref({std::make_shared<CondVar>(IsSpecifiedNode<schema::PrimitiveType_Mul>), mul1_ref, add_ref});
  return mul2_ref;
}
